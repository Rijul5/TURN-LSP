...
public class YangPathProvider {
  public static final PreferenceKey YANG_PATH = new PreferenceKey("yangPath", "");
  ...
  private List<IResourceDescription> yangPathDescriptions;
  
  public List<IResourceDescription> getYangPath(final Resource resource) {
    List<IResourceDescription> _elvis = null;
    if (this.yangPathDescriptions != null) {
      _elvis = this.yangPathDescriptions;
    } else {
      _elvis = (this.yangPathDescriptions = this.createYangPathDescriptions(resource));
    }
    return _elvis;
  }
  
  private List<IResourceDescription> createYangPathDescriptions(final Resource resource) {
    final Procedure2<IPreferenceValues, Resource> _function = (IPreferenceValues $0, Resource $1) -> {
      this.yangPathDescriptions = null;
    };
    ...
    final String yangpath = prefs.getPreference(YangPathProvider.YANG_PATH);
    final ArrayList<IResourceDescription> resourceDescriptions = CollectionLiterals.<IResourceDescription>newArrayList();
    boolean _isNullOrEmpty = StringExtensions.isNullOrEmpty(yangpath);
    ...
    if (_not) {
      ...
      Splitter.on(File.pathSeparator).split(yangpath).forEach(_function_1);
      final Consumer<Resource> _function_2 = (Resource it) -> {
        try {
          URI _uRI = it.getURI();
          String _plus = ("Indexing YangPath element: " + _uRI);
          ...
          if (_t instanceof Throwable) {
            ...
            String _plus_1 = ("Error indexing yangPath resource " + _string);
  ...
  private void process(final File file, final ResourceSet resourceSet) {
    try {
              ...
              while (entries.hasMoreElements()) {
                {
                  ...
                  if (((!entry.isDirectory()) && entry.getName().endsWith(".yang"))) {
      ...
      if (_t instanceof Throwable) {
        ...
        String _plus = ("Error loading yangPath resource " + _string);
